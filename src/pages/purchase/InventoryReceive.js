...
const InventoryReceive = () => {
  ...
  const [inventoryDetails, setInputList] = useState([]);
  ...
  const getAllInventoryReceive = useCallback(() => {
    setValidated(false);
    getData("Inventory?" + new URLSearchParams(searchCriteria), "GET");
  }, [getData, searchCriteria]);

  useEffect(() => {
    getAllInventoryReceive();
  }, [getAllInventoryReceive]);
    ...
    fetch(process.env.REACT_APP_API_URL + "Brand", {
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          let lastindex = inventoryDetails.length - 1;
  ...
  const saveInventoryReceive = (e) => {
    ...
    master.inventoryDetails = rowdataList;
    const methodType = master.id ? "PUT" : "POST";
    saveData("Inventory", methodType, master, getAllInventoryReceive);
  };
  const handleReset = () => {
  ...
  const removeMasterRowClick = (id) => {
    console.log(id);
    deleteData("Inventory/" + id, "DELETE", getAllInventoryReceive);
  };
  ...
  const headers = [
    { label: "Innventory Date", key: "inventoryDate" },
    { label: "Supplier", key: "supplierName" },
  ];

  return (
    <Fragment>
      <Actionbar pageTitle="Inventory Receive">
                    ...
                    <tbody>
                      ...
                      {res.data.map((item, index) => (
                        <tr
                          ...
                          onClick={() => {
                            setMaster(item);
                            setInputList(item.inventoryDetails);
                          }}
                          className={
                ...
                <Card>
                  <Card.Body>
                      ...
                      <Col sm={4}>
                        <Table responsive borderless>
                          <tbody align="right">
                            <tr>
                              <td>Receive Date :</td>
                              {master.inventoryDate && (
                                <td>{master.inventoryDate.split("T")[0]}</td>
                              )}
                            </tr>
                          </tbody>
                        </Table>
                    ...
                    <Table responsive bordered striped>
                      ...
                      {master.inventoryDetails && (
                        <tbody>
                          {master.inventoryDetails.map((item, index) => (
                            <tr key={index}>
      ...
      >
        <Offcanvas.Header closeButton className="header">
          <Offcanvas.Title className="headertextsize">Inventory Receive</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          {" "}
          <Card className="card">
                          ...
                          <Row className="mb-3">
                            <Form.Group as={Col}>
                              ...
                              <ProductGroup
                                ...
                                name="productGroupId"
                                selected={brands.productGroupId}
                              />
                            </Form.Group>
                ...
                <Card className="cardbgcolor">
                  <Row className="mb-1">
                    <Col className="col-md-3"> <Form.Control
                        ...
                        type="date"
                        name="inventoryDate"
                        value={master.inventoryDate.split("T")[0]}
